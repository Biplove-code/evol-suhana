...
import { Cn as useRevalidator, L as Link, Q as RouterContextProvider, an as useFetchers, fn as useLocation, gn as useNavigation, hn as useNavigate, ht as createContext$2, in as useFetcher, l as createRequestHandler$1, mn as useMatches } from "./chunk-S54KXAEJ-DkPASkOw.js";
...
function AnalyticsProvider({ canTrack: customCanTrack, cart: currentCart, children, consent, customData = {}, shop: shopProp = null, cookieDomain }) {
	...
	if (!!shop) {
		if (/\/68817551382$/.test(shop.shopId)) warnOnce("[h2:error:Analytics.Provider] - Mock shop is used. Analytics will not work properly.");
		else {
...
function hashKey(queryKey) {
	...
	let hash = "";
	for (const key of rawKeys) if (key != null) {
		if (typeof key === "object") hash += JSON.stringify(key);
...
function getRedirectUrl(requestUrl) {
	...
	if (redirectTo) {
		if (isLocalPath(requestUrl, redirectTo)) return redirectTo;
...
var SeoLogger = (0, import_react.lazy)(() => import("./log-seo-tags-IG37ONQ2-Bmu31eaV.js"));
function Seo({ debug }) {
